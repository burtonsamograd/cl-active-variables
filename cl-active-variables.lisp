...
(defpackage :cl-active-variables
  (:use :cl)
  (:nicknames "ACTIVE-VARIABLES")
...
(defmacro defactive (symbol value &key write-handler read-handler)
...
variable and call it's write handler, use (setf $symbol ...).  To read
...
the :read-handler and :write-handler keywords, or using setactive."
  (let ((setf-handler-name (gensym)))
    (alexandria:once-only (value write-handler read-handler)
      `(progn
	 ;; add the variable to the active variables list
	 (setf (gethash ',symbol *active-variables*) t)
	 ;; declare the variable and the value
	 ...
	 (defmacro ,setf-handler-name ()
	   (let ((read-handler (gensym)))
	     `(let ((,read-handler (get ',',symbol :read-handler)))
	      (if ,read-handler
		  (funcall ,read-handler ,',symbol)
		  ,',symbol))))
	 ;; define a handler to call the write handler for when you do a
	 ;; (setf $symbol ...)
	 (defsetf ,setf-handler-name () (new-val)
	   (let ((write-handler (gensym)))
	     `(let ((,write-handler (get ',',symbol :write-handler)))
		(when ,write-handler
		  (setf ,',symbol (funcall ,write-handler ,',symbol ,new-val))))))
	 ;; save all the info in the variable
	 (setf (get ',symbol :setf-handler-name) ',setf-handler-name)
	 (setf (get ',symbol :write-handler) ,write-handler)
	 (setf (get ',symbol :read-handler) ,read-handler)
	 ;; return the value
	 ,value))))
  
(defmacro setactive (symbol &key read-handler write-handler)
